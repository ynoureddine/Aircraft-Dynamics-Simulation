...
class Aircraft():
    ...
    def Calc_Forces(self, CONSTANTS, Atmosphere, height,TIME,SIM):
        self.forces.net_force_ENU = np.array([0, 0, 0])
        self.forces.net_force_ENU = self.forces.net_force_ENU + self.Calc_Lift(Atmosphere,height) 
        self.forces.net_force_ENU = self.forces.net_force_ENU + self.Calc_Drag(Atmosphere) 
        self.forces.net_force_ENU = self.forces.net_force_ENU + self.Calc_Thrust(CONSTANTS, Atmosphere,SIM) 
        self.forces.net_force_ENU = self.forces.net_force_ENU + self.Calc_Weight(height,TIME,SIM) 
    ...
    def Calc_Lift(self, Atmosphere,height):
    ...
    def Calc_Thrust(self, CONSTANTS, Atmosphere,SIM):
        ...
        if self.design.empty_mass > self.design.mass:
            thrust_0 = 0
            thrust_1 = 0
            thrust_2 = 0
    ...
    def Calc_Weight(self, height,TIME,SIM):
        current_velocity = (self.state.vel_ENU[1]*self.state.vel_ENU[1]+self.state.vel_ENU[0]*self.state.vel_ENU[0]+self.state.vel_ENU[2]*self.state.vel_ENU[2])**0.5
        self.design.mass = self.design.mass - self.design.max_fuel_consumption*SIM.DELTA_T*current_velocity/self.design.max_velocity    #estimate fuel burn as linearly proportion of current speed to top speed multiply by top speed fuel burn
        self.forces.weight_mag = self.design.mass * 3.98576e14/((height+6.371e6)**2)
